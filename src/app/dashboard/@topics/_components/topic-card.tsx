...
import { Calendar, Target, CheckCircle2, Clock, BookOpen, Plus } from "lucide-react"
...
function calculateProgress(topic: Topic): { completed: number; total: number; percentage: number } {
  if (!topic.learningObjectives) return { completed: 0, total: 0, percentage: 0 }
        ...
        <CardHeader className="pb-2 gap-0 ">
          <div className="flex items-center justify-between">
            ...
            {isEmpty ? (
              ...
              <Badge variant={progress.percentage === 100 ? "default" : "secondary"} className="shrink-0">
                {progress.percentage}%
              </Badge>
            ) : null}
          </div>
        ...
        <CardContent className="pt-0 h-64 flex flex-col justify-between">
          {isEmpty ? (
            <div className="flex flex-col items-center justify-center py-6 text-center bg-black/10 rounded-xl flex-1">
              ...
              <p className="text-sm text-muted-foreground mb-2">No learning objectives yet</p>
